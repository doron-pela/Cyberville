...
export default function GameLayout(){
    ...
    const {data: fetchedGame, isPending} = useGameForId(gameId);

    const rawDescription = fetchedGame?.["description"]?? "";
    const description = stringParse(rawDescription)
    const developers = fetchedGame?.["developers"]?? [];
    const tags = fetchedGame?.["tags"]?? [];
    const released = fetchedGame?.["released"]?? "";
    const stores = fetchedGame?.["stores"]?? [];
    ...
    return (
      <motion.div className={style.gameDetails} variants={pageVariants} initial={"initial"} animate={"animate"} exit={"exit"} key={gameId}>
        ...
        <div ref={reviewRef} className={style.reviewAnchor}>
          ...
          // fetchedGame?.["ratings"]||
            metacriticPlatforms={fetchedGame?.["metacritic_platforms"]}
            redditUrl={fetchedGame?.["reddit_url"]}
            ratings={fetchedGame?.["ratings"]}
            progress={progress}
            fixed={fixed}
          />
        </div>
