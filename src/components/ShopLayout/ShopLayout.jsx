...
export default function ShopLayout() {
  ...
  const { searchTerm, searchEnabled, setSearchTerm, setSearchEnabled } =
    useContext(SearchContext);

  const onCalendarOption =
    selected.key === "release dates" && selected.index === 2; //Helper boolean for if we are on our "Year Round" key in sidebar
    ...
    >
      ...
      >
        <h1 className={styles.title}>
          {onPlatformOption
            ? QueryKeys[selected.key][selected.index][1]
            : onCalendarOption
              ? QueryKeys[selected.key][selected.index][0]
              : QueryKeys[selected.key][selected.index]}{" "}
          Releases
        </h1>

        {onCalendarOption && (
          <ul className={styles.monthList}>
            {monthStrings.map((string, i) => (
              <li
                key={string}
                className={`${styles.month} ${i === monthIndex ? styles.selectedMonth : " "}`}
        ...
        <GameGrid
          ...
          data={queryToUse?.data}
          error={queryToUse?.error}
          isPending={queryToUse?.isPending}
          fetchNextPage={queryToUse?.fetchNextPage}
          hasNextPage={queryToUse?.hasNextPage}
          isFetchingNextPage={queryToUse?.isFetchingNextPage}
        />
      </section>
